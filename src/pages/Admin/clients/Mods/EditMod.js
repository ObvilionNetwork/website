...
class EditMod extends Component {
    ...
    componentDidMount() {
        ...
        fetch(Config.api_link + 'control/mod?id=' + urlParams.get('id'),
            {
                headers: {
                    Authorization: window.localStorage.getItem('token')
                }
            ...
            .then(async (d) => {
                ...
                if (out.error) {
                    this._updateData({ mod_not_found_error: true })
                    return console.error(out);
                }

                this._updateData({ mod: out.data, mod_icon: out.data.icon });
            })
            .catch(console.error)
    }

    confirmRemove() {
        fetch(Config.api_link + 'control/mod?id=' + this.state.mod.id,
            {
                method: 'DELETE',
                headers: {
                    Authorization: window.localStorage.getItem('token')
    ...
    render() {
        ...
        if (!urlParams.get('id')) {
            return (
                <Aux>
                    <Modal active setActive={ () => window.history.go(-1) }>
                        <h4 className='font-weight-bold'>
                            <i className='feather icon-alert-circle' style={{ color: '#d55562' }}/> Ошибка загрузки мода
                        </h4>

                        <h5>Не указан id редактируемого мода</h5>
        ...
        return (
            <Aux>
                ...
                <Modal active={ this.state.mod_not_found_error } setActive={() => window.history.go(-1)}>
                    <h4 className='font-weight-bold'>
                        <i className='feather icon-alert-circle' style={{ color: '#d55562' }}/> Ошибка загрузки мода
                    </h4>

                    <h5>Мод с указанным id не найден!</h5>
                    <hr style={{ marginBottom: '20px' }}/>
                ...
                <Modal active={ this.state.saved } setActive={() => window.history.go(-1)}>
                    <h4 className='font-weight-bold'>
                        <i className='feather icon-check' style={{ color: '#5be36d' }}/> Мод успешно обновлен
                    </h4>
                    ...
                    <Row className='mt-4' style={{marginLeft:'1px'}}>
                        ...
                        <Button className='but-cancel' onClick={() => this._updateData({ saved: false })}>Остаться</Button>
                    </Row>
                </Modal>
                ...
                <Row>
                    <Col>
                        <Card title="Редактирование мода">
                            <div className='fileUpload' onDrop={(e) => {
                                ...
                                {
                                    (() => {
                                        if (this.state.data.files.length <= 0 && !this.state.mod.path) {
                                            return [
                                        ...
                                        let name = this.state.mod.path + ' - ('
                                            + byteConverter(this.state.mod.fileSize, 2)
                                            + ')';

                                        if (this.state.data.files.length > 0) {
                                            name = this.state.data.files[0].name;
                                        }
                                ...
                                <Col md={6}>
                                    <Form>
                                        <Form.Group controlId="">
                                            ...
                                            <Form.Control id='name' placeholder="Введите название" defaultValue={this.state.mod.name} />
                                        </Form.Group>
                                    </Form>
                                </Col>
                                <Col md={6}>
                                ...
                                <Col md={6}>
                                    <Form>
                                        <Form.Group controlId="">
                                            ...
                                            <Form.Control defaultValue={this.state.mod.description} id='description' as="textarea" placeholder="Введите описание мода" rows="3" />
                                        </Form.Group>
                                    </Form>
                                </Col>
                                <Col md={6}>
                                    <Form.Group controlId="">
                                        ...
                                        <Form.Control id='category' as="select">
                                            {this.state.fromApi && this.state.fromApi.categories ? Object.keys(this.state.fromApi.categories).map(value => {
                                                return <option value={value} selected={this.state.mod.category === value}>{this.state.fromApi.categories[value]}</option>;
                                            }) : null}
                                        </Form.Control>
                                    </Form.Group>
                                </Col>
                                ...
                                <Col md={4}>
                                    <Form>
                                        <Form.Group controlId="">
                                            ...
                                            <Form.Control id='version' defaultValue={this.state.mod.version}>
                                    ...
                                    <Form.Group controlId="">
                                        ...
                                        <Form.Control id='clientType' as="select">
                                            {this.state.fromApi && this.state.fromApi.clientTypes ? Object.keys(this.state.fromApi.clientTypes).map(value => {
                                                return <option value={value} selected={this.state.mod.clientType === value}>{this.state.fromApi.clientTypes[value]}</option>;
                                            }) : null}
                                        </Form.Control>
                                    </Form.Group>
                                </Col>
                                    ...
                                    <Form.Group>
                                        ...
                                        <Form.Control id='clientVersion' as="select">
                                            {this.state.fromApi && this.state.fromApi.gameVersions ? this.state.fromApi.gameVersions.map(value => {
                                                return <option selected={this.state.mod.clientVersion === value}>{value}</option>;
                                            }) : null}
                                        </Form.Control>
                                    </Form.Group>
                                </Col>
